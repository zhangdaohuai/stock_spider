...
def test_pledge_detail(session: requests.Session) -> None:
    """验证1: 个股质押明细 (RPT_CSDC_LIST)"""
    print("=" * 60)
    print("验证1: 个股质押明细 — 603288")
    print("=" * 60)
    ...
    _print_result_summary(
        ...
        key_fields=[
            ...
            "TOTAL_SHARES", "PLEDGE_SHARES", "PLEDGE_RATIO",
        ],
    )
    time.sleep(_REQUEST_INTERVAL)


def test_pledge_institution(session: requests.Session) -> None:
    """验证2: 质押机构统计 (RPT_CSDC_DETAILED)"""
    print("=" * 60)
    print("验证2: 质押机构统计")
    print("=" * 60)
    data = _fetch_datacenter(
        session=session,
        report_name="RPT_CSDC_DETAILED",
        filter_expr='(SECURITY_CODE="603288")',
        sort_columns="END_DATE",
    )
    _print_result_summary(
        data,
        key_fields=[
            "SECURITY_CODE", "END_DATE", "PLEDGE_INSTITUTION",
            "PLEDGE_SHARES",
        ],
    )
    time.sleep(_REQUEST_INTERVAL)
...
def test_pledge_summary(session: requests.Session) -> None:
    """验证3: 质押比例统计 (RPT_CSDC_SUMMARY)"""
    print("=" * 60)
    print("验证3: 质押比例统计")
    print("=" * 60)
    data = _fetch_datacenter(
        session=session,
        report_name="RPT_CSDC_SUMMARY",
        filter_expr='(SECURITY_CODE="603288")',
        sort_columns="END_DATE",
    )
    _print_result_summary(
...
def main() -> None:
    """运行所有股权质押验证"""
    session = requests.Session()
    test_pledge_detail(session)
    test_pledge_institution(session)
    test_pledge_summary(session)
    session.close()
    print("\n股权质押验证全部完成 (3个API)")
